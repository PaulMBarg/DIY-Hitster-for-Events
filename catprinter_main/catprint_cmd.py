...
from catprinter_main.catprinter_utils import logger
from catprinter_main.catprinter_utils.cmds import PRINT_WIDTH, cmds_print_img
from catprinter_main.catprinter_utils.ble import run_ble
from catprinter_main.catprinter_utils.img import read_img, show_preview
...
def configure_logger(log_level):
    if not logger.hasHandlers():  # Check if the logger already has handlers
        ...
        logger.info("Logger is already configured. Skipping reconfiguration.")


def print_with_catprinter(args):
    ...
    if not os.path.exists(filename):
        ...
        raise FileNotFoundError(f"Codeimg not found at path {filename}.")
    ...
    try:
        ...
        logger.error(f'🛑 {e}')
        raise RuntimeError("Not able to read binary image or show preview.")
